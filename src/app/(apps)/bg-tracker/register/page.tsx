...
function RegisterForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Update profile with app_type (in case trigger didn't set it)
      if (authData.user) {
        await supabase
          .from('profiles')
          .update({ app_type: 'boardgame' })
      ...
      <div className="min-h-screen flex items-center justify-center p-4">
        <Card className="w-full max-w-md p-8 text-center" variant="glass">
          ...
          <div className="mt-6 flex flex-col gap-3">
            <Link href="/bg-tracker/login" className="btn-secondary btn-md inline-flex justify-center">
              Back to Login
            </Link>
          </div>
    ...
    <div className="min-h-screen flex items-center justify-center p-4">
      ...
      <Card className="relative z-10 w-full max-w-md p-8" variant="glass">
        ...
        <p className="mt-6 text-center text-sm text-slate-400">
          ...
          <Link href="/bg-tracker/login" className="text-wood-400 hover:text-wood-300 font-medium">
            Log in
          </Link>
        </p>
      </Card>
...
export default function BGRegisterPage() {
  return (
    <Suspense fallback={
