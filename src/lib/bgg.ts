...
import type { BGGSearchResult, BGGGameDetails, BGGCollectionItem } from '@/types';
...
// Rate limiting helper - BGG requires ~5 seconds between requests for complex queries, 
// allows more frequent simple ones, but we'll be safe.
// Using a global variable for rate limiting in serverless/edge environments isn't perfect
// but helps for single-instance or local dev.
...
export async function searchGames(query: string, token?: string): Promise<BGGSearchResult[]> {
    ...
    try {
        ...
        if (response.status === 401) {
            console.error('BGG API error: Unauthorized (Missing or invalid token)');
...
export async function getGameDetails(bggId: number, token?: string): Promise<BGGGameDetails | null> {
